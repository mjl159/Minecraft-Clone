...
int Graphics::genCubeTexture(std::string textureKey, glm::vec3 pos)
{
...
int Graphics::genBlockSides(std::string textureKey, std::bitset<6> sides, glm::vec3 pos) {
	int s;
	int sideCount = 0;
...
int Graphics::genWorld(std::vector<std::vector<std::vector<int>>> world, std::map<int, std::string> blockMap)
{
	int i, j, k, s;
	std::bitset<6> sides;
	...
	for(i = 0; i < world.size(); i++) {
		for(j = 0; j < world[i].size(); j++) {
			for(k = 0; k < world[i][j].size(); k++) {
				...
				if (world[i][j][k]) {
					genBlockSides(blockMap[world[i][j][k]], sides, glm::vec3(i, world[i].size() - j, k));
				}
			}
		}
	}
...
void Graphics::moveCamera(int action, GLfloat modifier)
{
	...
	case 0:
		...
		break;
	case 1:
		cameraPos = cameraPos - camera_move_speed * glm::cross(cameraFront, cameraUp);
		break;
	case 2:
		cameraPos = cameraPos - camera_move_speed * cameraFront;
		break;
...
void Graphics::look(GLfloat xOffset, GLfloat yOffset)
{
	...
	cameraFront = glm::vec3(glm::sin(glm::radians(theta_xz)) * glm::sin(glm::radians(theta_y)), glm::cos(glm::radians(theta_y)), glm::cos(glm::radians(theta_xz)) * glm::sin(glm::radians(theta_y)));
	model_view = glm::lookAt(cameraPos, cameraPos + cameraFront, cameraUp);
}